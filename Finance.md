...
[1,500 words on Notion](https://www.notion.so/Unconventional-views-against-scarcity-73613a3ff10449e589a9d2b7ff595492)
...
"Use" here isn't market speculation but the actual economic drive it propagates. Digital businesses, silk road 4.0?,
...
Oooouuuuuuuu fun! Partial differentials!!!!
I was wondering when I was gonna see one. Let's see what we have. You can find details in
`Finanace.pdf` which is separately rendered using `LaTex` or you can find the `tex` code [here]() .
Let's dig in. [After they attempted to apply the formula to the markets and incurred financial losses due to
lack of risk management in their trades, they decided to focus in their domain area, 
the academic environment."](https://en.wikipedia.org/wiki/Black%E2%80%93Scholes_model#cite_note-6) lololol
this phrase made me look at the 
[original paper](https://www.cs.princeton.edu/courses/archive/fall09/cos323/papers/black_scholes73.pdf)
 which is an actually good introduction to the subject.
 
### What is a call option? in this case?
"The simplest kind of option is one that gives the right to buy a single share of common stock". This is kind of like a
pinky swear promise but with fancy names. So what are we looking for here is simply the ability to buy/sell 
...
say that *sometimes* the *ability* to buy stock of Yara at $45 **may or may or not reflect the actual price of Yara stock at any
...
if we could reduce this phenomena to a *single dependent* of the form *price = some constraint* with respect
to time. The trouble arrives because we have **multiple dependent variables** of the form **price and maturity = some constraint** 
wrt to time.
The finance guys used to use this weird looking thing `C(S, T) = e^pT * SN(d_1)-(1-A)KN(d_2)` you can see the 
...
What does that mean exactly? The paper goes into how it's derived. I won't do that here. Let's solve some math.

### How to solve American style options?
Parameters given are (assuming the stock doesn't pay dividend):

- 1 month = 30 days.
- Stock price = $40
- call option strike= $45 in 4months.
- continuously-compounded risk-free rate = 3% per year
- stock mean return = 7% per year
